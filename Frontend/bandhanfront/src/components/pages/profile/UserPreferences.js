...
'Oriya', 'Punjabi', 'Sanskrit', 'Sindhi', 'Tamil', 'Telugu', 'Urdu', 'Arabic', 'Bhojpuri', 'Chhattisgarhi', 'Nepali', 'Rajasthani', 'Romani' ]
    ...
    return <>
        ...
        <Grid container component='form' noValidate  sx={{height:'86vh', backgroundColor:'white', padding:'1px', paddingTop:'1px',width: "100%" }} id='userpreference-form' onSubmit={handleSubmit}>
          <Grid item xs={6} display={"flex"} flexDirection={"column"} sx={{backgroundColor:"white", padding:'1px', paddingTop:'1px', color: "white"}}>
              ...
              <Select id="marital_status" name="marital_status" label='Marital Status'>{MARITAL_STATUS_CHOICES.map((choice) => (<MenuItem key={choice} value={choice}>{choice}</MenuItem>))}</Select>
              </FormControl>
              ...
              <Select id="motherTongue" name="motherTongue" label='Mother Tongue'>{LANGUAGE_CHOICES.map((motherTongue) => (<MenuItem key={motherTongue} value={motherTongue}>{motherTongue}</MenuItem>))}</Select>
              </FormControl>              
              ...
              <Select id="religion" name="religion" label='Religion'>{RELIGION_CHOICES.map((religion) => (<MenuItem key={religion} value={religion}>{religion}</MenuItem>))}</Select>
              </FormControl>              
          ...
          <Grid  item xs={6} display={"flex"} flexDirection={"column"}  sx={{backgroundColor:"white", padding:'0.5px', paddingTop:'1px', color: "white"}}>
              ...
              <Select id="familyStatus" name="familyStatus" label='Family Status'>{FAMILY_STATUS.map((familyStatus) => (<MenuItem key={familyStatus} value={familyStatus}>{familyStatus}</MenuItem>))}</Select>
              </FormControl>               
              ...
              <Select id="familyValues" name="familyValues" label='Family Values'>{FAMILY_VALUES.map((familyValues) => (<MenuItem key={familyValues} value={familyValues}>{familyValues}</MenuItem>))}</Select>
              </FormControl>              
